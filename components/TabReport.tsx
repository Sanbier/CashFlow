  ...
  return (
    <div className="space-y-6 animate-fadeIn mt-2">
      <div className="glass-panel p-6 rounded-[32px] relative overflow-hidden">
        <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-300/20 rounded-full blur-3xl -mr-10 -mt-10 pointer-events-none" />
        <h3 className="font-black text-slate-800 mb-8 flex items-center gap-2 border-b border-slate-200/50 pb-4 uppercase text-sm tracking-widest relative z-10">
          <PieChart size={18} className="text-indigo-600" /> Phân tích chi tiêu
        </h3>
              ...
              <div key={cat} className="animate-fadeIn">
                <div className="flex justify-between text-[11px] font-black mb-2 uppercase tracking-tight">
                  ...
                  <span className="text-slate-800">
                    {formatCurrency(amt)}{' '}
                    <span className={`${textColor} font-bold ml-1 opacity-80`}>({pct}%)</span>
                  </span>
                </div>
                <div className="w-full bg-white/40 rounded-full h-4 overflow-hidden border border-white/60 p-0.5 shadow-inner">
                  <div
                    className={`${progressBarColor} h-full rounded-full transition-all duration-1000 ease-out shadow-sm`}
